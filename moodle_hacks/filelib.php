...
{ //TODO: what is this? (skodak) - No idea (Fred)
    if ($str == '/' or empty($str)) {
        return '/';
    } else {
        return '/' . trim($str, '/') . '/';
...
function file_pluginfile($relativepath, $forcedownload, $preview = null, $offline = false, $embed = false)
{
    ...
    // ADDE FOR LOCAL CRIA TO SERVE FILES WITHOUT LOGIN
        ...
        $filename = array_pop($args);
        $filepath = $args ? '/' . implode('/', $args) . '/' : '/';
        if (!$file = $fs->get_file($context->id, $component, $filearea, $args[0], '/', $filename) or $file->is_directory()) {
            send_file_not_found();
        }
        // Send image files wihtout needing to login. Protect anyother file type
